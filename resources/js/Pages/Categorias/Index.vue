              ...
              <tr v-for="categoria in categoriasDocumentos" :key="categoria.id" class="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td class="px-6 py-4 text-right">
                  <div class="flex items-center justify-end space-x-1">
                    ...
                    <button v-if="categoria.raw.productos_count === 0" @click="toggleCategoria(categoria.id)" class="w-8 h-8 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors duration-150" title="Cambiar estado">
                      <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                      </svg>
                    </button>
                    <button v-if="categoria.estado === 'inactivo'" @click="confirmarEliminacion(categoria.id)" class="w-8 h-8 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors duration-150" title="Eliminar">
                      <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
            ...
            <div v-if="modalMode === 'details'" class="flex gap-2">
              <button v-if="selectedCategoria.productos_count === 0" @click="toggleCategoria(selectedCategoria.id)" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
                Cambiar Estado
              </button>
