...
const rentasDocumentos = computed(() => {
  return rentasData.value.map(r => {
    return {
      id: r.id,
      titulo: r.numero_contrato || `Renta #${r.id}`,
      subtitulo: r.cliente?.nombre || 'Sin cliente',
      estado: r.estado || 'activo',
      extra: `Equipos: ${r.equipos?.length || 0} | Inicio: ${r.fecha_inicio || 'N/A'}`,
      fecha: r.created_at,
      raw: r
    }
  })
...
const exportRentas = () => {
  ...
  const url = route('rentas.export') + (queryString ? `?${queryString}` : '')
  window.location.href = url
}

// Paginación
const paginationData = computed(() => ({
  current_page: rentasPaginator.value?.current_page || 1,
  last_page: rentasPaginator.value?.last_page || 1,
  per_page: rentasPaginator.value?.per_page || 10,
  from: rentasPaginator.value?.from || 0,
  to: rentasPaginator.value?.to || 0,
  total: rentasPaginator.value?.total || 0,
  prev_page_url: rentasPaginator.value?.prev_page_url,
  next_page_url: rentasPaginator.value?.next_page_url,
  links: rentasPaginator.value?.links || []
}))
...
const obtenerClasesEstado = (estado) => {
  const clases = {
    ...
    'proximo_vencimiento': 'bg-yellow-100 text-yellow-700',
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-start lg:items-center justify-between">
          ...
          <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto lg:flex-shrink-0">
            ...
            >
              ...
              <option value="numero_contrato-asc">Número A-Z</option>
              <option value="numero_contrato-desc">Número Z-A</option>
            </select>
          </div>
        </div>
      </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Renta</th>
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Cliente</th>
                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Equipos</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="renta in rentasDocumentos" :key="renta.id" class="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-700">{{ renta.raw.equipos?.length || 0 }} equipos</div>
                </td>
                <td class="px-6 py-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="space-y-3">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded-md">{{ selectedRenta.numero_contrato }}</p>
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Cliente</label>
                      <p class="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded-md">{{ selectedRenta.cliente?.nombre || 'N/A' }}</p>
                    </div>
                    <div>
