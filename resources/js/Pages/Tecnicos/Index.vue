            ...
            <div v-if="modalMode === 'confirm'">
              <div class="text-center">
                ...
                <p class="text-sm text-gray-500 mb-4">
                  ¿Estás seguro de que deseas eliminar el cliente <strong>{{ selectedTecnico?.nombre }} {{ selectedTecnico?.apellido }}</strong>?
...
const tecnicosDocumentos = computed(() => {
  return tecnicosData.value.map(t => {
    return {
      id: t.id,
      titulo: `${t.nombre} ${t.apellido}` || 'Sin nombre',
      subtitulo: t.email || '',
      estado: t.activo ? 'activo' : 'inactivo',
      extra: `Teléfono: ${t.telefono || 'N/A'}`,
      fecha: t.created_at,
      raw: t
    }
  })
...
const toggleTecnico = (id) => {
  ...
  if (!tecnico) return notyf.error('Cliente no encontrado')
