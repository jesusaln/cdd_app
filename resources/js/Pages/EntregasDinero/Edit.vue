<!-- /resources/js/Pages/EntregasDinero/Edit.vue -->
<script setup>
...
// Estado del formulario (prefill con la entrega)
const form = ref({
  fecha_entrega: props.entrega.fecha_entrega || new Date().toISOString().split('T')[0],
  monto_efectivo: props.entrega.monto_efectivo || 0,
  monto_transferencia: props.entrega.monto_transferencia || 0,
  monto_cheques: props.entrega.monto_cheques || 0,
  monto_tarjetas: props.entrega.monto_tarjetas || 0,
  notas: props.entrega.notas || ''
})
...
const submit = () => {
  ...
  router.put(route('entregas-dinero.update', { entregas_dinero: props.entrega.id }), {
    ...
    onSuccess: () => {
      notyf.success('Entrega actualizada correctamente')
      router.visit(route('entregas-dinero.index'))
    },
    onError: () => {
      notyf.error('Error al actualizar la entrega')
    }
  })
}
</script>
