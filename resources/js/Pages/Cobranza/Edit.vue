<!-- /resources/js/Pages/Cobranza/Edit.vue -->
<script setup>
...
// Formulario
const form = useForm({
  renta_id: props.cobranza.renta_id || '',
  fecha_cobro: props.cobranza.fecha_cobro ? new Date(props.cobranza.fecha_cobro).toISOString().split('T')[0] : '',
  monto_cobrado: props.cobranza.monto_cobrado || '',
  concepto: props.cobranza.concepto || 'mensualidad',
  estado: props.cobranza.estado || 'pendiente',
  fecha_pago: props.cobranza.fecha_pago ? new Date(props.cobranza.fecha_pago).toISOString().split('T')[0] : '',
  monto_pagado: props.cobranza.monto_pagado || '',
  metodo_pago: props.cobranza.metodo_pago || '',
  referencia_pago: props.cobranza.referencia_pago || '',
  notas: props.cobranza.notas || ''
})
...
const submit = () => {
  ...
  form.put(route('cobranza.update', props.cobranza.id), {
    onSuccess: () => {
      notyf.success('Cobranza actualizada exitosamente')
      router.visit(route('cobranza.index'))
    },
    onError: (errors) => {
      notyf.error('Error al actualizar la cobranza')
      saving.value = false
    },
    onFinish: () => {
      saving.value = false
...
<template>
  <Head title="Editar Cobranza" />

  <div class="cobranza-edit min-h-screen bg-gray-50">
    <div class="max-w-4xl mx-auto px-6 py-8">
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-8 mb-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-2xl font-bold text-slate-900">Editar Cobranza</h1>
            <p class="text-slate-600 mt-1">Modifica los datos de la cobranza</p>
          </div>
        </div>
      </div>
      ...
      <form @submit.prevent="submit" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <!-- Información de pago (solo si está pagado o parcial) -->
          <div v-if="form.estado === 'pagado' || form.estado === 'parcial'" class="md:col-span-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
                <p v-if="form.errors.fecha_pago" class="mt-1 text-sm text-red-600">
              ...
              <div>
                ...
                <p v-if="form.errors.monto_pagado" class="mt-1 text-sm text-red-600">
              ...
              <div>
                ...
                />
                <p v-if="form.errors.referencia_pago" class="mt-1 text-sm text-red-600">
        ...
        <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-200">
          ...
          >
            {{ saving ? 'Actualizando...' : 'Actualizar Cobranza' }}
          </button>
        </div>
      </form>
    </div>
