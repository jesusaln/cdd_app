          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="usuario in usuariosDocumentos" :key="usuario.id" class="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-700">{{ usuario.raw.roles?.map(r => r.name).join(', ') || 'Sin roles' }}</div>
                </td>
                <td class="px-6 py-4">
                  ...
                  <div class="space-y-3">
                    <div>
                      ...
                      <div class="mt-1 flex flex-wrap gap-2">
                        <span v-for="role in selectedUsuario.roles" :key="role.name"
...
const usuariosDocumentos = computed(() => {
  return usuariosData.value.map(u => {
    return {
      id: u.id,
      titulo: u.name || 'Sin nombre',
      subtitulo: u.email || '',
      estado: u.activo ? 'activo' : 'inactivo',
      extra: `Roles: ${u.roles?.map(r => r.name).join(', ') || 'Sin roles'}`,
      fecha: u.created_at,
      raw: u
    }
  })
