<template>
    ...
    <div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="producto in productosFiltrados" :key="producto.id" class="hover:bg-gray-100">
              ...
              <td class="px-4 py-3 text-sm text-gray-700">{{ producto.nombre }}</td>
              <td class="px-4 py-3 text-sm text-gray-700">{{ producto.precio_venta }}</td>
              <td class="px-4 py-3 text-sm text-gray-700">{{ producto.stock }}</td>
              <td class="px-4 py-3 text-sm text-gray-700">{{ producto.estado }}</td>
              <td class="px-4 py-3 flex space-x-2">
                ...
                <button @click="confirmarEliminacion(producto)" :disabled="loading" class="flex items-center space-x-2 bg-red-500 text-white px-4 py-2 rounded-md shadow-md hover:bg-red-600 transition duration-300">
      ...
      <ProductoModal v-if="isModalOpen" :producto="selectedProducto" :isOpen="isModalOpen" @close="closeModal" />
    </div>
  </template>
