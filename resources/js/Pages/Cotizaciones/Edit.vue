...
const form = useForm({
  ...
  productos: props.cotizacion.productos,
});

const buscarCliente = ref(props.cotizacion.cliente ? props.cotizacion.cliente.nombre_razon_social : '');
const buscarProducto = ref('');
const mostrarClientes = ref(false);
const mostrarProductos = ref(false);
const selectedProducts = ref(props.cotizacion.productos.map(p => p.id));
const quantities = ref(props.cotizacion.productos.reduce((acc, p) => {
  acc[p.id] = p.pivot.cantidad; // Usar pivot para obtener la cantidad
  return acc;
}, {}));
const prices = ref(props.cotizacion.productos.reduce((acc, p) => {
  acc[p.id] = p.pivot.precio; // Usar pivot para obtener el precio
  return acc;
}, {}));
const clienteSeleccionado = ref(props.cotizacion.cliente ? props.cotizacion.cliente.nombre_razon_social : null);
