    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Editar Cotización #{{ form.id }}</h1>
            <p class="text-gray-600">Actualiza la información de la cotización</p>
          </div>
...
const form = useForm({
  ...
  descuento_general: props.cotizacion.descuento_general,
  descuento_items: props.cotizacion.descuento_items,
  iva: props.cotizacion.iva,
  total: props.cotizacion.total,
  productos: [],
});
...
const cargarDatosCotizacion = () => {
  ...
  if (props.cotizacion.cliente) {
    clienteSeleccionado.value = props.cotizacion.cliente;
    form.cliente_id = props.cotizacion.cliente.id;
  }

  // Cargar productos
  if (props.cotizacion.productos) {
    props.cotizacion.productos.forEach(item => {
      // Usar pivot para obtener datos de la relación
      ...
      prices.value[key] = pivot.precio || 0;
      discounts.value[key] = pivot.descuento || 0;
    });
  }

  // Cargar descuento general
  descuentoGeneral.value = props.cotizacion.descuento_general || 0;
...
const actualizarCotizacion = () => {
  ...
  calcularTotal();
  form.put(route('cotizaciones.update', form.id), {
    onSuccess: () => {
