...
const props = defineProps({
  marcas: { type: [Object, Array], required: true },
...
// Datos
const marcasPaginator = computed(() => props.marcas)
const marcasData = computed(() => marcasPaginator.value?.data || [])
...
const marcasDocumentos = computed(() => {
  return marcasData.value.map(m => ({
    id: m.id,
    titulo: m.nombre || 'Sin nombre',
    subtitulo: m.descripcion ? `Descripción: ${m.descripcion.substring(0, 50)}${m.descripcion.length > 50 ? '...' : ''}` : 'Sin descripción',
    estado: m.estado || 'activo',
    extra: `Creada: ${new Date(m.created_at).toLocaleDateString('es-MX')}`,
    fecha: m.created_at,
    raw: m
  }))
})
...
const verDetalles = (doc) => {
  selectedMarca.value = doc.raw
  modalMode.value = 'details'
  showModal.value = true
}

const editarMarca = (id) => {
  router.visit(route('marcas.edit', id))
}
...
const eliminarMarca = () => {
  router.delete(route('marcas.destroy', selectedId.value), {
    preserveScroll: true,
    onSuccess: () => {
      notyf.success('Marca eliminada')
      showModal.value = false
      selectedId.value = null
      router.reload()
    },
    onError: (errors) => {
      notyf.error('No se pudo eliminar la marca')
    }
  })
}

const toggleMarca = (id) => {
  const marca = marcasData.value.find(m => m.id === id)
  if (!marca) return notyf.error('Marca no encontrada')
  const nuevoEstado = marca.estado === 'activo' ? 'inactivo' : 'activo'
  const mensaje = nuevoEstado === 'activo' ? 'Marca activada' : 'Marca desactivada'

  router.put(route('marcas.toggle', id), {
    preserveScroll: true,
    onSuccess: () => {
...
const exportMarcas = () => {
  ...
  const url = route('marcas.export') + (queryString ? `?${queryString}` : '')
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="marca in marcasDocumentos" :key="marca.id" class="hover:bg-gray-50 transition-colors duration-150">
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-900">{{ formatearFecha(marca.fecha) }}</div>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm font-medium text-gray-900">{{ marca.titulo }}</div>
                </td>
                <td class="px-6 py-4">
                  <div class="text-sm text-gray-700 max-w-xs truncate">{{ marca.raw.descripcion || 'Sin descripción' }}</div>
                </td>
                <td class="px-6 py-4">
                  <span :class="obtenerClasesEstado(marca.estado)" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium">
                    {{ obtenerLabelEstado(marca.estado) }}
                  </span>
                </td>
                <td class="px-6 py-4 text-right">
                  <div class="flex items-center justify-end space-x-1">
                    <button @click="verDetalles(marca)" class="w-8 h-8 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors duration-150" title="Ver detalles">
                      <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                      </svg>
                    </button>
                    <button @click="editarMarca(marca.id)" class="w-8 h-8 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition-colors duration-150" title="Editar">
                      <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      </svg>
                    </button>
                    <button v-if="marca.raw.productos_count === 0" @click="toggleMarca(marca.id)" class="w-8 h-8 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors duration-150" title="Cambiar estado">
                      <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                      </svg>
                    </button>
                    <button v-if="marca.estado === 'inactivo'" @click="confirmarEliminacion(marca.id)" class="w-8 h-8 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors duration-150" title="Eliminar">
                      <svg class="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <tr v-if="marcasDocumentos.length === 0">
                <td colspan="5" class="px-6 py-16 text-center">
                  <div class="flex flex-col items-center space-y-4">
                    ...
                    <div class="space-y-1">
                      <p class="text-gray-700 font-medium">No hay marcas</p>
                      <p class="text-sm text-gray-500">Las marcas aparecerán aquí cuando se creen</p>
                    </div>
                  </div>
                </td>
              </tr>
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div class="flex items-center justify-between p-6 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">
              {{ modalMode === 'details' ? 'Detalles de la Marca' : 'Confirmar Eliminación' }}
            </h3>
          ...
          <div class="p-6">
            <div v-if="modalMode === 'details' && selectedMarca">
              <div class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="space-y-3">
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Nombre</label>
                      <p class="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded-md">{{ selectedMarca.nombre }}</p>
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Estado</label>
                      <span :class="obtenerClasesEstado(selectedMarca.estado)" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium mt-1">
                        {{ obtenerLabelEstado(selectedMarca.estado) }}
                      </span>
                    </div>
                  </div>
                  <div class="space-y-3">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded-md">{{ formatearFecha(selectedMarca.created_at) }}</p>
                    </div>
                    <div>
                      <label class="block text-sm font-medium text-gray-700">Última actualización</label>
                      <p class="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded-md">{{ formatearFecha(selectedMarca.updated_at) }}</p>
                    </div>
                  </div>
                </div>
                <div v-if="selectedMarca.descripcion">
                  <label class="block text-sm font-medium text-gray-700">Descripción</label>
                  <p class="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded-md whitespace-pre-wrap">{{ selectedMarca.descripcion }}</p>
                </div>
              </div>
            </div>
            ...
            <div v-if="modalMode === 'confirm'">
              <div class="text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">¿Eliminar Marca?</h3>
                <p class="text-sm text-gray-500 mb-4">
                  ¿Estás seguro de que deseas eliminar la marca <strong>{{ selectedMarca?.nombre }}</strong>?
          ...
          <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
            ...
            <div v-if="modalMode === 'details'" class="flex gap-2">
              <button v-if="selectedMarca.productos_count === 0" @click="toggleMarca(selectedMarca.id)" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
                Cambiar Estado
              </button>
              <button @click="editarMarca(selectedMarca.id)" class="px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors">
                Editar
              </button>
            </div>
            <button v-if="modalMode === 'confirm'" @click="eliminarMarca" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
              Eliminar
            </button>
          </div>
        </div>
