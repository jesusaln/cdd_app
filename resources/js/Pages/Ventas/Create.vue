<template>
    ...
    <div class="ventas-create">
      ...
      <form @submit.prevent="crearVenta" class="space-y-6">
        ...
        <div class="flex justify-end space-x-4">
          ...
          <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600" :disabled="!form.cliente_id || selectedProducts.length === 0">Guardar Venta</button>
        </div>
      </form>
    </div>
  </template>
  ...
  const crearVenta = () => {
    form.productos = selectedProducts.value.map(id => ({ id, cantidad: quantities.value[id] || 1, precio: prices.value[id] || 0 }));
