<template>
    <Head title="Citas" />
    <div class="citas-index max-w-7xl mx-auto p-6 bg-gray-50 rounded-lg shadow-md">
      <h1 class="text-2xl font-semibold mb-6 text-center">Lista de Citas</h1>
      ...
      <div class="mb-6 flex flex-wrap gap-4">
        ...
        <select v-model="filtroEstado" class="border rounded px-3 py-2 flex-1 min-w-[200px]">
  ...
  const props = defineProps({
    citas: Array,
    flash: Object, // Para capturar mensajes flash como 'success'
  });
  ...
  const cambiarEstado = (cita) => {
    router.patch(route('citas.updateIndex', cita.id), { estado: cita.estado }, {
      ...
      preserveScroll: true,
      onSuccess: () => {
        notyf.success(props.flash?.success || 'Estado actualizado correctamente');
      },
      onError: () => {
        notyf.error('Error al actualizar el estado de la cita.');
        cita.estado = props.citas.find(c => c.id === cita.id).estado; // Revertir en caso de error
      },
    });
  };
