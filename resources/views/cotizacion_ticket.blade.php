{{-- resources/views/cotizacion_ticket.blade.php --}}
@php
...
<head>
    <meta charset="utf-8">
    <title>Ticket Cotización #{{ $cotizacion->numero_cotizacion }}</title>
    <style>
        @page {
...
<body>
    ...
    {{-- INFORMACIÓN --}}
    <div class="info">
        <div><strong>N°:</strong> {{ $cotizacion->numero_cotizacion }}</div>
        <div><strong>Fecha:</strong> {{ $cotizacion->created_at->format('d/m/Y H:i') }}</div>
        <div><strong>Cliente:</strong> {{ Str::limit($cotizacion->cliente->nombre_razon_social, 20) }}</div>
    ...
    <div class="totales">
        <div class="total-line">
            <span>Subtotal:</span>
            <span class="right">${{ $money($cotizacion->subtotal) }}</span>
        </div>
        @if (($cotizacion->descuento_general ?? 0) > 0)
            <div class="total-line">
                <span>Descuento:</span>
                <span class="right">- ${{ $money($cotizacion->descuento_general) }}</span>
            </div>
        @endif
        <div class="total-line">
            <span>IVA ({{ $configuracion->iva_porcentaje }}%):</span>
            <span class="right">${{ $money($cotizacion->iva) }}</span>
        </div>
    </div>
    ...
    <div class="center" style="font-size: 8px; margin: 5px 0;">
        VÁLIDA POR {{ $cotizacion->validez_dias ?? 30 }} DÍAS
    </div>
    ...
    @if ($cotizacion->notas)
        <div class="center" style="font-size: 7px; margin: 3px 0; border-top: 1px dashed #000; padding-top: 3px;">
            ...
            {{ Str::limit($cotizacion->notas, 100) }}
        </div>
    @endif
