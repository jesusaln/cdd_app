{{-- resources/views/pedido_pdf.blade.php --}}
@php
...
<head>
    ...
    <style>
        ...
        .cotizacion-left  { text-align: left; }
        .cotizacion-center{ text-align: center; }
        .cotizacion-right { text-align: right; }
    ...
    <div class="header avoid-break">
        ...
        <div class="cotizacion-bar">
            <div class="cotizacion-left">
                <b>Pedido:</b> #{{ $pedido->numero_pedido }}
            </div>
            <div class="cotizacion-center">
                <b>Fecha:</b> {{ $pedido->fecha ? $pedido->fecha->format('d/m/Y') : $pedido->created_at->format('d/m/Y') }}
            </div>
            <div class="cotizacion-right">
                <b>Estado:</b> {{ ucfirst($pedido->estado->label()) }}
            </div>
        </div>
    </div>
    ...
    <div class="info avoid-break">
        ...
        <div class="col">
            ...
            <table class="kv">
                <tr>
                    <td class="k">Razón Social:</td>
                    <td class="v">{{ $pedido->cliente->nombre_razon_social }}</td>
                </tr>
                @if ($pedido->cliente->rfc)
                <tr>
                    <td class="k">RFC:</td>
                    <td class="v">{{ $pedido->cliente->rfc }}</td>
                </tr>
                @endif
                @if ($pedido->cliente->calle || $pedido->cliente->numero_exterior || $pedido->cliente->colonia)
                <tr>
                    <td class="k">Dirección:</td>
                    <td class="v">
                        {{ trim(implode(' ', array_filter([
                            $pedido->cliente->calle,
                            $pedido->cliente->numero_exterior,
                            $pedido->cliente->numero_interior ? 'Int. ' . $pedido->cliente->numero_interior : null,
                            $pedido->cliente->colonia
                        ]))) }}
                    </td>
                </tr>
                @endif
                @if ($pedido->cliente->municipio || $pedido->cliente->estado || $pedido->cliente->codigo_postal)
                <tr>
                    <td class="k">Municipio:</td>
                    <td class="v">
                        {{ trim(implode(', ', array_filter([
                ...
                <tr>
                    <td class="k">Teléfono:</td>
                    <td class="v">{{ $pedido->cliente->telefono }}</td>
                </tr>
                @endif
                @if ($pedido->cliente->email)
                <tr>
                    <td class="k">Email:</td>
                    <td class="v">{{ $pedido->cliente->email }}</td>
                </tr>
                @endif
            </table>
        </div>
        ...
        <div class="right">
            <table class="totals-table">
                <tr>
                    ...
                    <td class="v">${{ $money($pedido->subtotal) }}</td>
                </tr>
                @if (($pedido->descuento_general ?? 0) > 0)
                    <tr>
                        <td class="k">Descuento:</td>
                        <td class="v">- ${{ $money($pedido->descuento_general) }}</td>
                    </tr>
                @endif
                <tr>
                    <td class="k">IVA:</td>
                    <td class="v">${{ $money($pedido->iva) }}</td>
                </tr>
            </table>
            <div class="grand">TOTAL &nbsp;&nbsp; ${{ $money($pedido->total) }}</div>
        </div>
    </div>
    ...
    <div class="fixed-footer">
        <div class="footer-grid">
            <div class="footer-col">
                <div class="footer-title">📝 Notas del Pedido</div>
                @if ($pedido->notas)
                    <p class="footer-text">{{ Str::limit($pedido->notas, 250) }}</p>
                @else
            ...
            <div class="footer-col">
                ...
                @if ($configuracion->banco || $configuracion->sucursal || $configuracion->cuenta || $configuracion->clabe || $configuracion->titular)
                    <div class="bank-info">
            ...
            <div class="footer-col">
                ...
                <div style="margin-top: 10px;">
                    ...
                    <p class="footer-text" style="margin: 2px 0 0 0; font-size: 6px; color: #666;">{{ $pedido->fecha ? $pedido->fecha->format('d/m/Y') : $pedido->created_at->format('d/m/Y') }}</p>
                </div>
            </div>
        </div>
    </div>
